...
public class Chatter {
	...
	private HashMap<String, String> hashmap; // a hashmap with a word as a key and a response as a value
	...
	public Chatter(String greeting, String goodbye, String responsesPath) {
		...
		this.hashmap = loadResponsesFromFile(responsesPath);
	...
	public String respondToPhrase(String userPhrase) {
		...
		Scanner userWords = new Scanner(userPhrase);

		// scans tokens in userPhrase for keys in the responses hashmap
		while (userWords.hasNext()) {
			compare = userWords.next();
			if (hashmap.containsKey(compare)) {
				useableKeys.add(hashmap.get(compare));
			}
		}
		// randomly selects a key if the userPhrase contains more than 0 keys
		if (!useableKeys.isEmpty()) {
			Random rand = new Random();
			int index = rand.nextInt(useableKeys.size());
			return useableKeys.get(index);
		} else {
			return "Please tell me more about " + compare;
		}
